...
class App(ctk.CTk):
    ...
    def _run_download(self, url):
        def on_progress(d):
            if d.get("status") == "downloading":
                ...
                self.after(0, lambda: self.dl_status.configure(text=f"Downloading... {pct_str}%"))
            elif d.get("status") == "finished":
                self.after(0, lambda: self.dl_status.configure(text="Processing..."))
        ...
        try:
            ...
            self.after(0, lambda: self.dl_status.configure(text=f"Done: {Path(path).name}"))
        except Exception as e:
            self.after(0, lambda: self.dl_status.configure(text=f"Error: {e}"))
    ...
    def _run_convert(self, input_path, target_format):
        def on_progress(status_text, percent):
            ...
            self.after(0, lambda: self.conv_status.configure(text=f"{status_text} {percent:.0%}"))
        ...
        try:
            ...
            self.after(0, lambda: self.conv_status.configure(text=f"Done: {Path(out_path).name}"))
        except Exception as e:
            self.after(0, lambda: self.conv_status.configure(text=f"Error: {e}"))
